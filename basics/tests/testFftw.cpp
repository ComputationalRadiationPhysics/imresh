...
void testFftw(SDL_Renderer * rpRenderer)
{
    ...
    testFftwAndPrint( rpRenderer,axes, [](float x){return std::sin(1*x);}, "Sin(x)"  ); axes.y += 110;
    testFftwAndPrint( rpRenderer,axes, [](float x){return std::sin(2*x);}, "Sin(2x)" ); axes.y += 110;
    testFftwAndPrint( rpRenderer,axes, [](float x){return std::sin(3*x);}, "Sin(3x)" ); axes.y += 110;
    testFftwAndPrint( rpRenderer,axes, [](float x){return std::abs(x);  }, "|x|"     ); axes.y += 110;
    testFftwAndPrint( rpRenderer,axes, [](float x){return x>0?1:0;      }, "Theta(x)" );
}
